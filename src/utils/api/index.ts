    ...
    for (const k in queryFilter) {
      ...
      if (typeof value === 'object') {
        ...
        if (value.in) {
          ...
          where[k] = {
            in: arr,
            // mode: typeof arr[0] === 'string' ? 'insensitive' : 'default',
          };
        }
        // if (value.not) {
...
export function deleteNotAllowedKeys(obj: obj, allowedKeys: string[]) {
  for (const key in obj) {
...
export const deepConsole = (json: any) =>
  console.dir(json, {
    showHidden: true,
    depth: 99,
    colors: true,
...
export const getSumDaysOfDates = (start_date: Date, end_date?: Date | null) => {
